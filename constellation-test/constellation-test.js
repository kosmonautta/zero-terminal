...
// Small stacked-rectangle "node window" icons echo the linked hypertext-map
// look — a couple of little document chips in the corner of each resolved
// formation's box, standing in for the linked cards a formation was built
// out of. Count is derived from how many afterimages it actually required,
// capped so the icon cluster never gets absurd for the bigger formations.
function nodeIcons(count){
    const shown = Math.max(1, Math.min(count, 3));
    let html = "";
    for(let i = 0; i < shown; i++){
        html += `<span class="cc-icon" style="--i:${i}"></span>`;
    }
    return html;
}
...
function renderChartedLog(){
    ...
    chartedCards.forEach(card => {
        ...
        const reqCount = Array.isArray(card.requires) ? card.requires.length : 1;

        const el = document.createElement("div");
        el.className = "const-card" + (isActive ? " active" : "");
        el.innerHTML = `
            <div class="cc-icons">${nodeIcons(reqCount)}</div>
...
// tape: an earth-date timestamp, a live RECORDING state, and TARGET / TYPE /
// BATTERY fields. Every value is real state (session date, current lock
// count, match name, charted-formation completion) rather than decoration —
// the HUD is just the field's own data, wearing the tape's vocabulary.
function formatHudDate(d){
...
function updateHud(){
    ...
    if(dateEl) dateEl.textContent = formatHudDate(new Date());
    ...
    const battLabelEl = document.getElementById("hudBatteryLabel");
    if(battEl){
        const pct = Math.round((chartedCards.length / TOTAL_FORMATIONS) * 100);
        battEl.style.width = pct + "%";
        if(battLabelEl) battLabelEl.textContent = pct <= 20 ? "LOW" : (pct >= 90 ? "FULL" : pct + "%");
    }
}
